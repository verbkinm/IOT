...
uint64_t hostBroadCastCheckSum(const Host_Broadcast *body)
{
    if (body == NULL)
        return 0;
...
uint64_t hostBroadCastSize(const Host_Broadcast *body)
{
    if (body == NULL)
        return 0;
...
uint64_t hostBroadCastToData(const Host_Broadcast *body, char *outData, uint64_t outDataSize)
{
