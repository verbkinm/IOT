...
public:
    ...
    static std::forward_list<std::pair<IOTV_Event *, IOTV_Action *>> parseJson(const QByteArray &data, const std::forward_list<const Base_Host *> &hosts);
    static QByteArray toData(const std::forward_list<std::pair<IOTV_Event *, IOTV_Action *>> &list);
