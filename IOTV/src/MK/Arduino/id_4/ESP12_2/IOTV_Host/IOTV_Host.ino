...
uint8_t readType[NUMBER_READ_CHANNEL] = {DATA_TYPE_INT_32, DATA_TYPE_BOOL, DATA_TYPE_INT_8, DATA_TYPE_BOOL};
uint8_t *writeType = readType;
...
void loop() 
{
  ...
  // Записываем сренее значение ADC в канал чтения
  auto resADC = avrADC();
  ...
  !!!
  if (iot.readChannel[REPEATE].data && !(resADC > ADC_BORDER)) // iot.readChannel[ADC] > ADC_BORDER) - идёт воспроизведение
  {
    // Делаем задержку 
    delay(3000);

    // И ещё раз проверяем состояние воспроизведения, если мелодия не воспроизводится, устанавливаем флаг переключения состояния воспроизведения
    resADC = avrADC();
    memcpy(iot.readChannel[ADC].data, &resADC, iot.readChannel[ADC].dataSize);
    if (resADC < ADC_BORDER)
      *(iot.readChannel[TRIGGER].data) = true;
  }

  // Если установлен флаг переключения состояния воспроизведения
  if (iot.readChannel[TRIGGER]) 
  {
    // Подаём логическую единицу на ножку PLAY_PIN
    pinMode(PLAY_PIN, OUTPUT);
    digitalWrite(PLAY_PIN, HIGH);
    
    // Если мелодия воспроизводится, делаем задержку на 3500 мс (для выключения), иначе на 500 мс
    if (iot.readChannel[ADC] > ADC_BORDER)
      delay(3500);
    else
      delay(500);
    ...
    // Снимаем флаг 
    iot.readChannel[TRIGGER] = false;
  } 
  
  // Если установлен режим отличный от -1
  if ((iot.readChannel[MODE] > -1) && (iot.readChannel[MODE] < 3)) 
  {
    Serial.print("set mode: ");
    Serial.println(iot.readChannel[MODE]);

    // Устанавливаем логическую единицу на ножке соответствующей переданному режиму
    pinMode(modeArr[iot.readChannel[MODE]], OUTPUT);
    digitalWrite(modeArr[iot.readChannel[MODE]], HIGH);
    delay(500);
    // Устанавливаем логический ноль на ножке соответствующей переданному режиму и переключаем режим на INPUT
    digitalWrite(modeArr[iot.readChannel[MODE]], LOW);
    pinMode(modeArr[iot.readChannel[MODE]], INPUT);

    // Убираем присутствие переключения режима
    iot.readChannel[MODE] = -1;
  }
}
